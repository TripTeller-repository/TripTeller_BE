...
<details>
<summary>데이터 선택 추출, 정렬, 필터링 - utils/feed-extractor.ts</summary>
<div markdown="1">
...
<details>
<summary>서버 실행 환경에 따른 쿠키 환경 설정 - authentication/auth.controller.ts</summary>
<div markdown="1">
...
<details>
<summary>리퀘스트 로깅 처리 - middlewares/logger.middleware.ts</summary>
<div markdown="1">
...
<details>
<summary>에러 로깅 처리 - utils/all-exceptions.filter.ts</summary>
<div markdown="1">
...
- ### 2. 사전 기획과 관계없이 Join(populate)을 수행할 때 상위 모델의 id 값을 하위 모델의 필드에 일관되게 넣어야 하는 것의 중요성을 인식함.
  ...
  - 본 프로젝트에서 검색 API를 구현할 때 연결된 하위모델의 필드에 상위모델의 ID값이 없는 경우가 일부 있었음. 그 결과 여러 계층을 반복적으로 탐색해야 했기 때문에 비효율성이 발생함.
